"""Reacher visualisations for paper Fig. 2.

Renders five plots from a single trained Reacher run, into outputs/results/:
  - fig2_reacher_wm_x.png       — true vs WM next-x dynamics in (θ₁, θ₂)
    space (3D surface).
  - fig2_reacher_wm_y.png       — same, for next-y.
  - fig2_reacher_trajectory.png — policy-conditioned trajectory rollouts:
    true vs WM dynamics, per goal.
  - fig2_reacher_q_theta.png    — Q vs Q^true at fixed action over (θ₁, θ₂)
    at ω₁=ω₂=0.
  - fig2_reacher_q_omega.png    — same, over (ω₁, ω₂) at θ₁=θ₂=0.

Reads seed_0 of a Reacher sweep — does NOT retrain. The Q plots additionally
run VI on the saved PQN checkpoint (a few seconds).

Usage:
    # Auto: trains a fresh Reacher sweep (1 seed by default), then renders.
    uv run python scripts/reacher_visualisation.py

    # Reuse an existing sweep (skips training).
    uv run python scripts/reacher_visualisation.py \\
        --sweep_dir outputs/reacher/seeds_<TS>
"""

import argparse
import glob as _glob
import importlib
import json
import os
import pathlib
import pickle
import subprocess
import sys
from datetime import datetime

import jax
import jax.numpy as jnp
import numpy as np
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import matplotlib.gridspec
from matplotlib.ticker import MaxNLocator

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))


_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def _train_sweep(config_stem, num_seeds):
    """Run PQN+WM training for `num_seeds` seeds; return the sweep dir."""
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    sweep_dir = os.path.join(_REPO_ROOT, "outputs", config_stem, f"seeds_{ts}")
    seeds_csv = ",".join(str(i) for i in range(num_seeds))
    cmd = [
        sys.executable, os.path.join(_REPO_ROOT, "run.py"),
        "--config", f"configs/{config_stem}.py",
        "--phases", "pqn,wm",
        "--seeds", seeds_csv,
        "--sweep_dir", sweep_dir,
    ]
    print(f"\n=== Auto-training {config_stem} "
          f"({num_seeds} seed{'s' if num_seeds > 1 else ''}) ===")
    print(" ".join(cmd))
    ret = subprocess.run(cmd)
    if ret.returncode != 0:
        raise RuntimeError(
            f"run.py failed for {config_stem} (exit {ret.returncode})"
        )
    return sweep_dir


def _resolve_seed_dir(sweep_dir):
    """Return <sweep_dir>/seed_0/ — Fig 2 only consumes one seed."""
    seed0 = pathlib.Path(sweep_dir) / "seed_0"
    if not seed0.is_dir():
        raise FileNotFoundError(f"No seed_0/ under {sweep_dir}")
    return str(seed0)


def _resolve_wm_dir(seed_dir):
    """Pick the most recent wm_* subdir inside a seed dir."""
    subs = sorted(
        p for p in pathlib.Path(seed_dir).iterdir()
        if p.is_dir() and p.name.startswith("wm_")
    )
    if not subs:
        raise FileNotFoundError(f"No wm_* subdir under {seed_dir}")
    return str(subs[-1])


def _build_dynamics(args, ENV_CONFIG, WM_CONFIG, p_params):
    """Construct true and WM dynamics functions in 4D effective state."""
    import gymnax
    from envs.env_dynamics import make_env_dynamics_fn

    basic_env, env_params = gymnax.make("MountainCar-v0")  # placeholder shape
    from envs.reacher import Reacher
    basic_env = Reacher()
    env_params = basic_env.default_params

    from envs.reacher_utils import (
        make_reacher_effective_dynamics_fn, make_reacher_wm_dynamics_fn,
        reacher_effective_to_obs,
    )
    true_dyn_fn = make_reacher_effective_dynamics_fn(basic_env, env_params)
    wm_dyn_fn = make_reacher_wm_dynamics_fn(p_params, WM_CONFIG, ENV_CONFIG["ACTION_DIM"])
    return true_dyn_fn, wm_dyn_fn, reacher_effective_to_obs


def _save_pair(V_left, V_right, x_data, y_data, x_lo, x_hi, y_lo, y_hi,
               xticks, yticks, x_label, y_label,
               z_left, z_right, save_path, zticks=None):
    """Two paired 3D-surface plots side-by-side, shared (vmin, vmax).

    PIL post-crop + inter-panel tightening to remove the whitespace
    matplotlib leaves around 3D axes. ``zticks=None`` uses an auto-locator
    (MaxNLocator(nbins=4)); pass an explicit list to fix z-ticks.
    """
    from plotting.style import CMAP
    from matplotlib.transforms import ScaledTranslation
    import io as _io
    from PIL import Image as _Image

    vmin = min(float(V_left.min()), float(V_right.min()))
    vmax = max(float(V_left.max()), float(V_right.max()))
    fig, axes = plt.subplots(
        1, 2, figsize=(12, 4.6),
        subplot_kw={"projection": "3d"},
        layout="constrained",
    )
    fig.get_layout_engine().set(
        w_pad=0.01, h_pad=0.01, wspace=0.0, hspace=0.0,
    )
    extra = []
    for col, (V, z_label) in enumerate([
        (V_left, z_left), (V_right, z_right),
    ]):
        ax = axes[col]
        ax.plot_surface(
            x_data, y_data, V, cmap=CMAP, edgecolor="none",
            alpha=0.9, vmin=vmin, vmax=vmax,
        )
        ax.set_xlabel(x_label, fontsize=22, labelpad=10)
        ax.set_ylabel(y_label, fontsize=22, labelpad=10)
        ax.set_zlim(vmin, vmax)
        ax.set_xticks(xticks)
        ax.set_yticks(yticks)
        ax.set_xlim(x_lo, x_hi)
        ax.set_ylim(y_lo, y_hi)
        if zticks is None:
            ax.zaxis.set_major_locator(MaxNLocator(nbins=4))
        else:
            ax.set_zticks(zticks)
        ax.tick_params(axis="both", labelsize=16)
        ax.set_zlabel(z_label, fontsize=20, labelpad=10, rotation=90)
        ax.zaxis.label.set_transform(
            ax.zaxis.label.get_transform()
            + ScaledTranslation(0, -0.12, fig.dpi_scale_trans)
        )
        extra.extend([ax.zaxis.label, ax.xaxis.label, ax.yaxis.label])

    fig.canvas.draw()
    buf = _io.BytesIO()
    fig.savefig(buf, format="png", dpi=300, bbox_inches="tight",
                pad_inches=0.0, bbox_extra_artists=extra)
    plt.close(fig)
    buf.seek(0)
    img = _Image.open(buf).convert("RGB")
    arr = np.asarray(img)

    # Outer tight bbox.
    content = (arr < 250).any(axis=-1)
    if content.any():
        rows = np.where(content.any(axis=1))[0]
        cols = np.where(content.any(axis=0))[0]
        margin = 2
        top = max(0, int(rows[0]) - margin)
        bot = min(arr.shape[0], int(rows[-1]) + 1 + margin)
        left = max(0, int(cols[0]) - margin)
        right = min(arr.shape[1], int(cols[-1]) + 1 + margin)
        arr = arr[top:bot, left:right]
    # Inter-panel tightening: split, tight-crop halves, rejoin with fixed gap.
    col_has_content = (arr < 250).any(axis=-1).any(axis=0)
    keep_gap_px = 55
    if col_has_content.any():
        is_white = ~col_has_content
        in_run, run_start, runs = False, 0, []
        for i, w in enumerate(is_white):
            if w and not in_run:
                run_start, in_run = i, True
            elif not w and in_run:
                in_run = False
                runs.append((run_start, i))
        if in_run:
            runs.append((run_start, len(is_white)))
        interior = [r for r in runs if r[0] > 0 and r[1] < arr.shape[1]]
        if interior:
            gs, ge = max(interior, key=lambda r: r[1] - r[0])
            split = (gs + ge) // 2
            left_half, right_half = arr[:, :split], arr[:, split:]

            def _tight_col_range(half):
                cont = (half < 250).any(axis=-1).any(axis=0)
                if not cont.any():
                    return 0, half.shape[1]
                cs = np.where(cont)[0]
                return int(cs[0]), int(cs[-1]) + 1

            L_lo, L_hi = _tight_col_range(left_half)
            R_lo, R_hi = _tight_col_range(right_half)
            gap = np.full((arr.shape[0], keep_gap_px, 3), 255, dtype=np.uint8)
            arr = np.concatenate(
                [left_half[:, L_lo:L_hi], gap, right_half[:, R_lo:R_hi]],
                axis=1,
            )
    _Image.fromarray(arr).save(save_path)
    print(f"  Saved {save_path}")


def _make_paper_heatmaps(true_dyn_fn, wm_dyn_fn, ENV_CONFIG, out_dir,
                          heatmap_res=40, action_idx=8,
                          grid_axis="theta",
                          theta_fixed=(0.0, 0.0),
                          omega_fixed=(0.0, 0.0),
                          file_suffix=""):
    """Two 3D plots, each 1x2 (WM left, true right):
        (i)  next x in (grid₁, grid₂)
        (ii) next y in (grid₁, grid₂)

    ``grid_axis`` chooses which pair to grid over:
      - ``"theta"`` → vary (θ₁, θ₂), fix ω at ``omega_fixed`` (default slice).
      - ``"omega"`` → vary (ω₁, ω₂), fix θ at ``theta_fixed``.

    ``file_suffix`` is appended before ".png" so non-default slices land in
    distinct files (e.g. ``"_omega1"`` for the ω=(1,1) variant).
    """
    state_ranges = ENV_CONFIG["STATE_RANGES"]
    n = heatmap_res * heatmap_res

    if grid_axis == "theta":
        lo, hi = state_ranges[0]
        g1 = jnp.linspace(lo, hi, heatmap_res)
        g2 = jnp.linspace(lo, hi, heatmap_res)
        G1, G2 = jnp.meshgrid(g1, g2, indexing="ij")
        base = jnp.stack([
            G1.flatten(), G2.flatten(),
            jnp.full(n, float(omega_fixed[0])),
            jnp.full(n, float(omega_fixed[1])),
        ], axis=-1)
        x_label, y_label = r"$\theta_1$", r"$\theta_2$"
        z_var = r"\theta"
        ticks = [-2, 0, 2]
    elif grid_axis == "omega":
        lo, hi = state_ranges[2]
        g1 = jnp.linspace(lo, hi, heatmap_res)
        g2 = jnp.linspace(lo, hi, heatmap_res)
        G1, G2 = jnp.meshgrid(g1, g2, indexing="ij")
        base = jnp.stack([
            jnp.full(n, float(theta_fixed[0])),
            jnp.full(n, float(theta_fixed[1])),
            G1.flatten(), G2.flatten(),
        ], axis=-1)
        x_label, y_label = r"$\omega_1$", r"$\omega_2$"
        z_var = r"\omega"
        ticks = [-2, 0, 2]
    else:
        raise ValueError(f"grid_axis must be 'theta' or 'omega', got {grid_axis!r}")

    actions = jnp.full(n, action_idx)
    s_true = np.asarray(jax.block_until_ready(true_dyn_fn(base, actions)))
    s_wm = np.asarray(jax.block_until_ready(wm_dyn_fn(base, actions)))

    # Next-state fingertip under true and WM dynamics.
    nx_t = (np.cos(s_true[:, 0]) + np.cos(s_true[:, 1])).reshape(G1.shape)
    ny_t = (np.sin(s_true[:, 0]) + np.sin(s_true[:, 1])).reshape(G1.shape)
    nx_w = (np.cos(s_wm[:, 0]) + np.cos(s_wm[:, 1])).reshape(G1.shape)
    ny_w = (np.sin(s_wm[:, 0]) + np.sin(s_wm[:, 1])).reshape(G1.shape)

    G1_np = np.asarray(G1)
    G2_np = np.asarray(G2)
    g_lo, g_hi = float(lo), float(hi)

    # Next x.
    _save_pair(
        nx_w, nx_t, G1_np, G2_np,
        g_lo, g_hi, g_lo, g_hi, ticks, ticks,
        x_label, y_label,
        rf"$P_x({z_var})$", rf"$P_x^{{\mathrm{{true}}}}({z_var})$",
        os.path.join(out_dir, f"fig2_reacher_wm_x{file_suffix}.png"),
    )
    # Next y.
    _save_pair(
        ny_w, ny_t, G1_np, G2_np,
        g_lo, g_hi, g_lo, g_hi, ticks, ticks,
        x_label, y_label,
        rf"$P_y({z_var})$", rf"$P_y^{{\mathrm{{true}}}}({z_var})$",
        os.path.join(out_dir, f"fig2_reacher_wm_y{file_suffix}.png"),
    )


# ── Q-value comparison heatmaps (Fig 2 — companion to the WM heatmaps) ─────


def _run_vi_one_goal(PQN_CONFIG, ENV_CONFIG, q_params, q_batch_stats,
                     goal, mask):
    """Returns (axis_grids, Q_pi, Q_pqn) on the 4D effective Reacher grid.

    Q_pi  shape: (action_dim,) + grid_shape — true Q^π under PQN's greedy
                  policy, via policy evaluation.
    Q_pqn shape: (action_dim,) + grid_shape — PQN-net Q values on the grid.
    """
    from eval.value_iteration import (
        precompute_dynamics, precompute_rewards_dones,
        bellman_iteration, evaluate_pqn_on_grid,
    )
    from envs.reacher import Reacher
    from envs.reacher_utils import (
        make_reacher_effective_dynamics_fn, reacher_effective_to_obs,
    )

    env = Reacher(
        reward_type=PQN_CONFIG["REWARD_TYPE"],
        sigma=PQN_CONFIG["REWARD_SIGMA"],
        a=PQN_CONFIG["REWARD_A"],
        max_steps_in_episode=PQN_CONFIG["MAX_STEPS_IN_EPISODE"],
        torque_values=PQN_CONFIG.get("REACHER_TORQUE_VALUES"),
    )
    env_params = env.default_params
    dynamics_fn = make_reacher_effective_dynamics_fn(env, env_params)

    state_ranges = ENV_CONFIG["VI_STATE_RANGES"]
    vi_grid_res = ENV_CONFIG["VI_GRID_RES"]
    vi_max_iter = ENV_CONFIG["VI_MAX_ITER"]
    convergence_threshold = ENV_CONFIG.get("VI_CONVERGENCE_THRESHOLD", 0.0)
    state_dim = ENV_CONFIG["VI_STATE_DIM"]
    obs_state_dim = PQN_CONFIG["STATE_DIM"]
    action_dim = PQN_CONFIG["ACTION_DIM"]
    gamma = PQN_CONFIG["GAMMA"]
    reward_type = PQN_CONFIG["REWARD_TYPE"]
    sigma = PQN_CONFIG["REWARD_SIGMA"]
    a_threshold = PQN_CONFIG["REWARD_A"]
    terminate_on_goal = PQN_CONFIG["TERMINATE_ON_GOAL"]

    print("  Precomputing 4D dynamics...")
    dyn_fn = jax.jit(precompute_dynamics, static_argnums=(0, 1, 2, 3))
    axis_grids, all_next_states = dyn_fn(
        dynamics_fn, vi_grid_res,
        tuple(tuple(r) for r in state_ranges), action_dim,
    )
    all_next_states = jax.block_until_ready(all_next_states)

    print("  Precomputing rewards/dones for goal...")
    precompute_fn = jax.jit(
        precompute_rewards_dones, static_argnums=(3, 6, 7, 8, 9)
    )
    all_r, all_d = precompute_fn(
        all_next_states, goal, mask, reward_type, sigma, a_threshold,
        state_dim, terminate_on_goal, None, reacher_effective_to_obs,
    )

    print("  Evaluating PQN on grid (Q per action)...")
    _, Q_pqn = evaluate_pqn_on_grid(
        q_params, q_batch_stats, goal, mask, axis_grids,
        PQN_CONFIG, action_dim, state_dim,
        state_to_obs_fn=reacher_effective_to_obs,
        obs_state_dim=obs_state_dim,
    )
    Q_pqn = jax.block_until_ready(Q_pqn)

    print("  Policy evaluation under PQN policy (Q^true)...")
    pi_pqn = jnp.argmax(Q_pqn, axis=0).astype(jnp.int32)
    bellman_fn = jax.jit(bellman_iteration, static_argnums=(5, 6, 7))
    _, Q_pi, _ = bellman_fn(
        all_next_states, all_r, all_d, axis_grids,
        gamma, vi_max_iter, action_dim, convergence_threshold, pi_pqn,
    )
    Q_pi = jax.block_until_ready(Q_pi)
    return axis_grids, np.asarray(Q_pi), np.asarray(Q_pqn)


def _make_paper_q_heatmaps(PQN_CONFIG, ENV_CONFIG, q_params, q_batch_stats,
                           out_dir, goal_idx, action_name,
                           theta_fixed=(0.0, 0.0),
                           omega_fixed=(0.0, 0.0),
                           file_suffix="",
                           plots=("theta", "omega"),
                           _cache=None):
    """Two 3D plots, each 1x2 (Q left, Q^true right), for one fixed action:
        (i)  Q over (θ₁, θ₂) at ω = ``omega_fixed``  (default (0, 0))
        (ii) Q over (ω₁, ω₂) at θ = ``theta_fixed``  (default (0, 0))

    ``file_suffix`` is appended before ".png" so non-default slices land in
    distinct files (e.g. ``"_at1"`` for the (θ=1, ω=1) variant).

    ``_cache`` (optional dict) lets the caller reuse the expensive VI tensor
    across multiple slice variants. Pass an empty dict on the first call;
    subsequent calls with the same dict skip the VI re-computation.
    """
    goal = PQN_CONFIG["GOALS"][goal_idx]
    mask = PQN_CONFIG["REWARD_MASK"][goal_idx]
    print(f"  Goal {goal_idx}: goal={np.asarray(goal)}  mask={np.asarray(mask)}")

    if _cache is not None and "axis_grids" in _cache:
        axis_grids = _cache["axis_grids"]
        Q_pi = _cache["Q_pi"]
        Q_pqn = _cache["Q_pqn"]
        print("  Reusing cached Q tensors.")
    else:
        axis_grids, Q_pi, Q_pqn = _run_vi_one_goal(
            PQN_CONFIG, ENV_CONFIG, q_params, q_batch_stats, goal, mask,
        )
        if _cache is not None:
            _cache["axis_grids"] = axis_grids
            _cache["Q_pi"] = Q_pi
            _cache["Q_pqn"] = Q_pqn
    action_names = ENV_CONFIG["ACTION_NAMES"]
    if action_name not in action_names:
        raise ValueError(
            f"action_name {action_name!r} not in ACTION_NAMES ({action_names})"
        )
    a = action_names.index(action_name)
    print(f"  Using action {a}  ({action_name})")

    def _idx_closest(grid, val):
        return int(np.argmin(np.abs(np.asarray(grid) - val)))
    i_w0 = _idx_closest(axis_grids[2], float(omega_fixed[0]))
    i_w1 = _idx_closest(axis_grids[3], float(omega_fixed[1]))
    i_t0 = _idx_closest(axis_grids[0], float(theta_fixed[0]))
    i_t1 = _idx_closest(axis_grids[1], float(theta_fixed[1]))

    T1, T2 = np.meshgrid(
        np.asarray(axis_grids[0]), np.asarray(axis_grids[1]), indexing="ij",
    )
    W1, W2 = np.meshgrid(
        np.asarray(axis_grids[2]), np.asarray(axis_grids[3]), indexing="ij",
    )
    theta_lo, theta_hi = float(axis_grids[0][0]), float(axis_grids[0][-1])
    omega_lo, omega_hi = float(axis_grids[2][0]), float(axis_grids[2][-1])
    theta_ticks = [-2, 0, 2]
    omega_ticks = [-1, 0, 1]
    theta_zticks = [0.6, 0.8, 1.0]
    omega_zticks = [0.4, 0.6, 0.8]

    if "theta" in plots:
        _save_pair(
            Q_pqn[a, :, :, i_w0, i_w1], Q_pi[a, :, :, i_w0, i_w1], T1, T2,
            theta_lo, theta_hi, theta_lo, theta_hi,
            theta_ticks, theta_ticks,
            r"$\theta_1$", r"$\theta_2$",
            r"$Q(\theta)$", r"$Q^{\mathrm{true}}(\theta)$",
            os.path.join(out_dir, f"fig2_reacher_q_theta{file_suffix}.png"),
            zticks=theta_zticks,
        )
    if "omega" in plots:
        _save_pair(
            Q_pqn[a, i_t0, i_t1, :, :], Q_pi[a, i_t0, i_t1, :, :], W1, W2,
            omega_lo, omega_hi, omega_lo, omega_hi,
            omega_ticks, omega_ticks,
            r"$\omega_1$", r"$\omega_2$",
            r"$Q(\omega)$", r"$Q^{\mathrm{true}}(\omega)$",
            os.path.join(out_dir, f"fig2_reacher_q_omega{file_suffix}.png"),
            zticks=omega_zticks,
        )


# ── Trajectory plot (Fig 1 right) ──────────────────────────────────────────


def _break_at_wrap(traj, d0, d1, jump_threshold=3.0):
    """Insert NaN rows between consecutive points where |Δdim| > threshold.

    Prevents matplotlib from drawing a straight line across the figure when a
    trajectory wraps around ±π on an angle dim. Threshold in radians; 3.0
    catches wrap jumps (~2π) but not normal motion.
    """
    traj = np.asarray(traj)
    if traj.shape[0] < 2:
        return traj
    deltas = np.abs(np.diff(traj[:, [d0, d1]], axis=0))
    wraps = np.where(np.any(deltas > jump_threshold, axis=1))[0]
    if len(wraps) == 0:
        return traj
    out = [traj[: wraps[0] + 1]]
    nan_row = np.full((1, traj.shape[1]), np.nan)
    for i, w in enumerate(wraps):
        out.append(nan_row)
        next_end = wraps[i + 1] + 1 if i + 1 < len(wraps) else traj.shape[0]
        out.append(traj[w + 1 : next_end])
    return np.concatenate(out, axis=0)


def _plot_true_vs_wm(ax, true_traj, pred_traj, d0, d1, color):
    """True (dashed) vs WM (solid, on top) trajectory pair, same color."""
    true_line = _break_at_wrap(true_traj, d0, d1)
    pred_line = _break_at_wrap(pred_traj, d0, d1)
    ax.plot(true_line[:, d0], true_line[:, d1],
            color=color, linewidth=4.0, linestyle="--", dashes=(3.0, 2.0),
            zorder=4)
    ax.plot(pred_line[:, d0], pred_line[:, d1],
            color=color, linewidth=4.5, zorder=6)


def _true_vs_wm_legend():
    from matplotlib.lines import Line2D
    return [
        Line2D([0], [0], color="gray", linewidth=4.0, linestyle="--",
               dashes=(3.0, 2.0), label="true"),
        Line2D([0], [0], color="gray", linewidth=4.5, label="WM"),
    ]


def _plot_trajectory_policy(
    per_goal_true,
    per_goal_pred,
    starts,
    goals,
    goal_masks,
    state_labels,
    state_ranges,
    dim_pair,
    n_y=1,
    n_x=1,
    reward_type=None,
    reward_a=None,
    reward_sigma=None,
    save_path="trajectory_policy.png",
):
    """Policy-conditioned trajectories: each panel = one starting state, all
    goals overlaid in different colors. Goal targets marked with lines and a
    shaded region showing the reward threshold (a for sparse, sigma for
    gaussian).

    Layout: rows = n_y (d1 values), cols = n_x (d0 values).
    """
    from matplotlib.lines import Line2D
    from plotting.style import PALETTE

    d0, d1 = dim_pair
    n_goals = len(per_goal_true)
    # Paper view: zoom around the reachable fingertip square.
    xlim = (-1.55, 1.3)
    ylim = (-1.55, 1.3)
    xticks = (-1, 0, 1)
    yticks = (-1, 0, 1)

    n_total = n_y * n_x
    fig, axes = plt.subplots(
        1, n_total, figsize=(5 * n_total, 5), squeeze=False,
    )

    def _truncate_at_goal(traj, goal_arr, mask_arr, half_w):
        """Truncate trajectory at the first step where the goal is reached."""
        if half_w <= 0:
            return traj
        mask_dims = np.where(np.array(mask_arr) > 0)[0]
        if len(mask_dims) == 0:
            return traj
        diffs = traj[:, mask_dims] - np.array(goal_arr)[mask_dims]
        dist = np.sqrt((diffs ** 2).sum(axis=-1))
        in_goal = dist <= half_w
        if not np.any(in_goal):
            return traj
        first_in = int(np.argmax(in_goal))
        return traj[: first_in + 1]

    for idx in range(n_total):
        ax = axes[0, idx]
        ax.grid(True, alpha=0.15)

        for g in range(n_goals):
            color = PALETTE[g % len(PALETTE)]
            true_traj = np.array(per_goal_true[g][idx])
            pred_traj = np.array(per_goal_pred[g][idx])

            if reward_type in ("sparse", "sparse_negative"):
                _half_w = reward_a if reward_a else 0
            elif reward_type == "gaussian":
                _half_w = reward_sigma if reward_sigma else 0
            else:
                _half_w = 0
            true_traj = _truncate_at_goal(true_traj, goals[g], goal_masks[g], _half_w)
            pred_traj = _truncate_at_goal(pred_traj, goals[g], goal_masks[g], _half_w)

            _plot_true_vs_wm(ax, true_traj, pred_traj, d0, d1, color)

            # Goal marker: line/circle + shaded region for reward threshold.
            goal_arr = np.array(goals[g])
            mask_arr = np.array(goal_masks[g])
            half_w = _half_w
            if mask_arr[d0] > 0 and mask_arr[d1] > 0 and half_w > 0:
                circle = plt.Circle((goal_arr[d0], goal_arr[d1]), half_w,
                                    color=color, alpha=0.18, fill=True,
                                    zorder=3)
                ax.add_patch(circle)
                ax.add_patch(plt.Circle(
                    (goal_arr[d0], goal_arr[d1]), half_w,
                    color=color, alpha=0.6, fill=False, linewidth=1.2,
                    zorder=3,
                ))
                ax.plot(goal_arr[d0], goal_arr[d1], "+", color=color,
                        markersize=10, markeredgewidth=2.0, zorder=3)
            elif mask_arr[d0] > 0:
                gv = goal_arr[d0]
                ax.axvline(gv, color=color, linewidth=1.5, alpha=0.5, linestyle=":")
                if half_w > 0:
                    ax.axvspan(gv - half_w, gv + half_w, color=color, alpha=0.08)
                    ax.axvline(gv - half_w, color=color, linewidth=0.8, alpha=0.5)
                    ax.axvline(gv + half_w, color=color, linewidth=0.8, alpha=0.5)
            elif mask_arr[d1] > 0:
                gv = goal_arr[d1]
                ax.axhline(gv, color=color, linewidth=1.5, alpha=0.5, linestyle=":")
                if half_w > 0:
                    ax.axhspan(gv - half_w, gv + half_w, color=color, alpha=0.08)
                    ax.axhline(gv - half_w, color=color, linewidth=0.8, alpha=0.5)
                    ax.axhline(gv + half_w, color=color, linewidth=0.8, alpha=0.5)

        ax.plot(float(starts[idx, d0]), float(starts[idx, d1]),
                "ko", markersize=7, zorder=7)

        ax.set_xlim(*xlim)
        ax.set_ylim(*ylim)
        ax.set_xticks(list(xticks))
        ax.set_yticks(list(yticks))
        ax.tick_params(axis="both", labelsize=14)
        ax.set_aspect("equal")
        ax.set_xlabel(state_labels[d0], fontsize=16)
        if idx == 0:
            ax.set_ylabel(state_labels[d1], fontsize=16)
        else:
            ax.set_ylabel("")
            ax.tick_params(left=False, labelleft=False)

    handles = []
    if reward_type in ("sparse", "sparse_negative", "gaussian"):
        handles.append(Line2D([0], [0], marker="P", color="black",
                              linestyle="", markersize=17,
                              markerfacecolor="black", markeredgecolor="black",
                              alpha=0.7, label="goal"))
    handles.extend(_true_vs_wm_legend())
    axes[0, 0].legend(handles=handles, fontsize=17, loc="lower left",
                      labelcolor="black", handlelength=1.8, borderpad=0.45,
                      handletextpad=0.55, labelspacing=0.4)

    fig.tight_layout(pad=0.2, w_pad=2.2)
    fig.savefig(save_path, dpi=300, bbox_inches="tight", pad_inches=0.02)
    plt.close(fig)


def _fp_to_obs(target_fp):
    """Inverse FK: pick a (theta_1, theta_2) pair whose fingertip is near target_fp.

    Solves cos(t1)+cos(t2)=fp_x, sin(t1)+sin(t2)=fp_y via sum-to-product.
    Returns a 6D obs [t1, t2, 0, 0, fp_x_actual, fp_y_actual].
    """
    fp_x, fp_y = target_fp
    r = float(np.sqrt(fp_x ** 2 + fp_y ** 2))
    if r >= 2.0:
        r = 1.99
    A = float(np.arctan2(fp_y, fp_x))
    B = float(np.arccos(r / 2.0))
    t1 = A + B
    t2 = A - B
    # Wrap to [-pi, pi].
    t1 = (t1 + np.pi) % (2 * np.pi) - np.pi
    t2 = (t2 + np.pi) % (2 * np.pi) - np.pi
    fp_x_a = float(np.cos(t1) + np.cos(t2))
    fp_y_a = float(np.sin(t1) + np.sin(t2))
    return np.array([t1, t2, 0.0, 0.0, fp_x_a, fp_y_a])


def _theta_omega_to_obs(t1, t2, w1, w2):
    """Build a 6D obs from explicit (θ₁, θ₂, ω₁, ω₂); fp via FK."""
    fp_x = float(np.cos(t1) + np.cos(t2))
    fp_y = float(np.sin(t1) + np.sin(t2))
    return np.array([float(t1), float(t2), float(w1), float(w2), fp_x, fp_y])


def _make_paper_trajectory_policy(
    p_params, q_params, q_batch_stats, basic_env, env_params,
    ENV_CONFIG, WM_CONFIG, PQN_CONFIG, out_dir,
    target_a_fp=(0.2, 0.0), target_b_fp=(-1.2, -1.0),
    state_a=None, state_b=None,
    goal_indices_override_arg=None,
):
    """Trajectory policy plot for paper: 2 specific starts.

    Each start is built either from an explicit (θ₁, θ₂, ω₁, ω₂) tuple
    (state_a/state_b) if provided, or from an inverse-FK target fingertip
    position with ω=0 (target_a_fp/target_b_fp).
    """
    from envs.env_dynamics import make_env_dynamics_fn
    from envs.reacher_utils import (
        reacher_obs_to_effective, reacher_effective_to_obs,
    )
    from eval.world_model import compute_policy_trajectory_rollouts
    from training.wm import make_world_model

    state_dim = ENV_CONFIG["STATE_DIM"]  # 6 for Reacher obs
    action_dim = ENV_CONFIG["ACTION_DIM"]
    state_labels = list(ENV_CONFIG["STATE_LABELS"])
    # LaTeX x/y instead of fp_x/fp_y for paper plots.
    state_labels[4] = r"$x$"
    state_labels[5] = r"$y$"
    state_ranges = ENV_CONFIG["STATE_RANGES"]
    goals = PQN_CONFIG["GOALS"]
    goal_masks = PQN_CONFIG["REWARD_MASK"]
    goal_indices_override = (
        goal_indices_override_arg
        if goal_indices_override_arg is not None
        else ENV_CONFIG.get("WM_TRAJECTORY_GOAL_INDICES")
    )
    print(f"  goal_indices_override = {goal_indices_override}")

    def _resolve_start(state_tuple, fp_target):
        if state_tuple is not None:
            return _theta_omega_to_obs(*state_tuple), state_tuple, None
        return _fp_to_obs(fp_target), None, fp_target

    start_a_obs, state_a_resolved, fp_a = _resolve_start(state_a, target_a_fp)
    start_b_obs, state_b_resolved, fp_b = _resolve_start(state_b, target_b_fp)
    starts_obs = jnp.array(np.stack([start_a_obs, start_b_obs]))
    src_a = (f"θω={state_a_resolved}" if state_a_resolved is not None
             else f"target_fp={fp_a}")
    src_b = (f"θω={state_b_resolved}" if state_b_resolved is not None
             else f"target_fp={fp_b}")
    print(f"  Start A: {src_a} -> obs {np.round(np.array(starts_obs[0]), 3)}")
    print(f"  Start B: {src_b} -> obs {np.round(np.array(starts_obs[1]), 3)}")

    dynamics_fn = make_env_dynamics_fn(basic_env, env_params, "Reacher")
    # WM uses effective state dim (4D for Reacher), not obs dim (6D).
    wm_dim = WM_CONFIG.get("WM_OUTPUT_DIM", state_dim)
    p_model = make_world_model(WM_CONFIG, wm_dim)

    def sample_fn(_key, _n):
        return starts_obs

    per_goal_true, per_goal_pred, starts_p, n_y_p, n_x_p, goal_idxs = (
        compute_policy_trajectory_rollouts(
            p_params, p_model, q_params, q_batch_stats, PQN_CONFIG,
            state_dim, action_dim, state_ranges, dynamics_fn,
            slice_point=jnp.zeros(state_dim), dim_pair=(4, 5),
            goals=goals, goal_masks=goal_masks,
            n_steps=PQN_CONFIG.get("MAX_STEPS_IN_EPISODE", 100),
            n_x=2, n_y=1, wm_config=WM_CONFIG,
            sample_states_fn=sample_fn,
            state_to_eff_fn=reacher_obs_to_effective,
            eff_to_obs_fn=reacher_effective_to_obs,
            goal_indices_override=goal_indices_override,
        )
    )
    plot_goals = goals[goal_idxs]
    plot_masks = goal_masks[goal_idxs]

    save_path = os.path.join(out_dir, "fig2_reacher_trajectory.png")
    _plot_trajectory_policy(
        per_goal_true, per_goal_pred, starts_p,
        goals=plot_goals, goal_masks=plot_masks,
        state_labels=state_labels, state_ranges=state_ranges,
        dim_pair=(4, 5), n_y=n_y_p, n_x=n_x_p,
        reward_type=PQN_CONFIG.get("REWARD_TYPE"),
        reward_a=PQN_CONFIG.get("REWARD_A"),
        reward_sigma=PQN_CONFIG.get("REWARD_SIGMA"),
        save_path=save_path,
    )
    print(f"  Saved {save_path}")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--sweep_dir", default=None,
                        help="seeds_<TS> dir from a Reacher run. If omitted, "
                             "a fresh sweep is trained (see --num_seeds). "
                             "Reads seed_0/wm_<latest>/ from the sweep.")
    parser.add_argument("--num_seeds", type=int, default=1,
                        help="Seeds to train when auto-training. Ignored when "
                             "--sweep_dir is passed. Fig 2 only consumes "
                             "seed_0, so the default of 1 is sufficient.")
    parser.add_argument("--out_dir", default="outputs/results",
                        help="Where to write the 3 paper PNGs. Default: "
                             "outputs/results/ (shared with other figure scripts).")
    parser.add_argument("--config", default="reacher",
                        help="Config module to load (bare module name, "
                             "e.g. 'reacher'). Auto-train uses "
                             "configs/<config>.py.")
    parser.add_argument("--heatmap_action", type=int, default=8,
                        help="Action index for the heatmap row (default: (+1,+1) = 8).")
    parser.add_argument("--vi_grid_res", type=int, default=None,
                        help="Override ENV_CONFIG['VI_GRID_RES'] for both the "
                             "WM and Q heatmaps (default: config value, "
                             "typically 50). "
                             "Higher = smoother Q surface but quartically "
                             "more 4D grid cells for value iteration.")
    parser.add_argument("--q_plots", default="theta,omega",
                        help="Comma-sep subset of {theta, omega} for the Q "
                             "heatmap. Default produces both.")
    parser.add_argument("--skip_trajectory", action="store_true",
                        help="Skip trajectory_policy plot (e.g. if PQN checkpoint not available).")
    parser.add_argument("--traj_target_a", type=float, nargs=2, default=(0.2, 0.0),
                        metavar=("FP_X", "FP_Y"),
                        help="Target fingertip pos for first start. "
                             "Ignored if --traj_state_a is set.")
    parser.add_argument("--traj_target_b", type=float, nargs=2, default=(-1.2, -1.0),
                        metavar=("FP_X", "FP_Y"),
                        help="Target fingertip pos for second start. "
                             "Ignored if --traj_state_b is set.")
    parser.add_argument("--traj_state_a", type=float, nargs=4,
                        default=(-1.2, 2.2, -0.8, -0.4),
                        metavar=("T1", "T2", "W1", "W2"),
                        help="Explicit (θ₁, θ₂, ω₁, ω₂) for first start.")
    parser.add_argument("--traj_state_b", type=float, nargs=4,
                        default=(-2.7, -1.8, 0.2, -0.2),
                        metavar=("T1", "T2", "W1", "W2"),
                        help="Explicit (θ₁, θ₂, ω₁, ω₂) for second start.")
    parser.add_argument("--traj_goal_idxs", type=int, nargs="+", default=None,
                        help="Override WM_TRAJECTORY_GOAL_INDICES (which goals to overlay).")
    parser.add_argument("--q_goal_idx", type=int, default=1,
                        help="Goal index used for the Q-value comparison heatmaps.")
    parser.add_argument("--q_action_name", default="(1,1)",
                        help="Action label (from ENV_CONFIG[ACTION_NAMES]) used "
                             "for the Q-value comparison heatmaps.")
    args = parser.parse_args()

    config_mod = importlib.import_module(f"configs.{args.config}")
    ENV_CONFIG = config_mod.ENV_CONFIG
    ENV_NAME = config_mod.ENV_NAME
    assert ENV_NAME == "Reacher", f"This script is Reacher-specific (got {ENV_NAME})"

    if args.sweep_dir is None:
        args.sweep_dir = _train_sweep(args.config, args.num_seeds)
    seed_dir = _resolve_seed_dir(args.sweep_dir)
    wm_dir = _resolve_wm_dir(seed_dir)
    print(f"Reading WM from {wm_dir}")

    with open(os.path.join(wm_dir, "wm_config.json")) as f:
        WM_CONFIG = json.load(f)

    wm_ckpt_path = os.path.join(wm_dir, "wm_checkpoint.pkl")
    if not os.path.exists(wm_ckpt_path):
        # Arch-sweep wm_track layout: nested step_*/wm_checkpoint.pkl.
        candidates = sorted(_glob.glob(
            os.path.join(wm_dir, "step_*", "wm_checkpoint.pkl")
        ))
        if not candidates:
            raise FileNotFoundError(f"No WM checkpoint at {wm_ckpt_path}")
        wm_ckpt_path = candidates[-1]
    with open(wm_ckpt_path, "rb") as f:
        wm_ckpt = pickle.load(f)
    p_params = wm_ckpt["params"]
    print(f"Loaded WM from {wm_ckpt_path}")

    out_dir = args.out_dir
    os.makedirs(out_dir, exist_ok=True)

    true_dyn_fn, wm_dyn_fn, _ = _build_dynamics(args, ENV_CONFIG, WM_CONFIG, p_params)

    print("\n── Plot 1: paper dynamics heatmaps at slice ω = (1, -1) ──")
    # Vary θ at ω = (1, -1). The omega-grid variant is intentionally not
    # produced for WM (was deemed less informative); the Q plots still
    # produce both grid axes via _make_paper_q_heatmaps. WM heatmap grid
    # res matches the VI grid res so all surfaces in Fig 2 use the same
    # sampling density.
    effective_grid_res = args.vi_grid_res or ENV_CONFIG["VI_GRID_RES"]
    _make_paper_heatmaps(true_dyn_fn, wm_dyn_fn, ENV_CONFIG, out_dir,
                         effective_grid_res, args.heatmap_action,
                         grid_axis="theta", omega_fixed=(1.0, -1.0))

    if not args.skip_trajectory:
        pqn_ckpt_path = os.path.join(seed_dir, "pqn_checkpoint.pkl")
        if not os.path.exists(pqn_ckpt_path):
            print(f"\nSkipping trajectory_policy plot: no PQN checkpoint at {pqn_ckpt_path}")
        else:
            with open(pqn_ckpt_path, "rb") as f:
                pqn_ckpt = pickle.load(f)
            q_params = pqn_ckpt["params"]
            q_batch_stats = pqn_ckpt.get("batch_stats", {})
            # Use the run's saved JSON config (live config may have different
            # arch hyperparams or goal definitions).
            saved_pqn_path = os.path.join(seed_dir, "pqn_config.json")
            with open(saved_pqn_path) as f:
                PQN_CONFIG = json.load(f)
            # JSON stores arrays as nested lists; convert back to jnp.
            for k in ("GOALS", "REWARD_MASK"):
                if k in PQN_CONFIG:
                    PQN_CONFIG[k] = jnp.array(PQN_CONFIG[k])

            from envs.reacher import Reacher
            basic_env = Reacher(
                reward_type=PQN_CONFIG["REWARD_TYPE"],
                sigma=PQN_CONFIG["REWARD_SIGMA"],
                a=PQN_CONFIG["REWARD_A"],
                max_steps_in_episode=PQN_CONFIG["MAX_STEPS_IN_EPISODE"],
                torque_values=PQN_CONFIG.get("REACHER_TORQUE_VALUES"),

            )
            env_params = basic_env.default_params

            print("\n── Plot 2: paper trajectory_policy (2 starts in fp space) ──")
            _make_paper_trajectory_policy(
                p_params, q_params, q_batch_stats, basic_env, env_params,
                ENV_CONFIG, WM_CONFIG, PQN_CONFIG, out_dir,
                target_a_fp=tuple(args.traj_target_a),
                target_b_fp=tuple(args.traj_target_b),
                state_a=(tuple(args.traj_state_a)
                         if args.traj_state_a is not None else None),
                state_b=(tuple(args.traj_state_b)
                         if args.traj_state_b is not None else None),
                goal_indices_override_arg=args.traj_goal_idxs,
            )

            if args.vi_grid_res is not None:
                print(f"  Overriding VI_GRID_RES "
                      f"{ENV_CONFIG['VI_GRID_RES']} → {args.vi_grid_res}")
                ENV_CONFIG = dict(ENV_CONFIG)
                ENV_CONFIG["VI_GRID_RES"] = args.vi_grid_res
            print("\n── Plot 3: paper Q-value heatmaps at (θ, ω) = (1, -1) ──")
            q_plot_set = tuple(p.strip() for p in args.q_plots.split(","))
            _make_paper_q_heatmaps(
                PQN_CONFIG, ENV_CONFIG, q_params, q_batch_stats, out_dir,
                goal_idx=args.q_goal_idx, action_name=args.q_action_name,
                theta_fixed=(1.0, -1.0), omega_fixed=(1.0, -1.0),
                plots=q_plot_set,
            )


if __name__ == "__main__":
    main()
