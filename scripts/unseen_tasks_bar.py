...
Reads Reacher and MountainCar sweeps (produced by run.py --config …) and
renders a side-by-side bar chart of the discounted returns under the
optimal (R*) and WM-derived (R^WM) policies for the three unseen goals in
each environment.
...
For each (env, goal) bar:
  - Height = mean over the seeds of the per-seed mean.
  - Error bar = pooled std across all seeds × 512 returns, computed via
...
Usage:
    # Auto: trains fresh Reacher + MC-position sweeps, then renders.
    uv run python scripts/unseen_tasks_bar.py --num_seeds 1   # cheap
    uv run python scripts/unseen_tasks_bar.py --num_seeds 10  # paper
...
Outputs:
    outputs/results/fig3_unseen.png
"""
...
def _train_sweep(config_stem: str, num_seeds: int) -> str:
    """Run PQN+WM+unseen for `num_seeds` seeds; return the sweep dir."""
...
def _aggregate_env(env_name: str, sweep_dir: str, goals, n_seeds: int = 10):
    ...
    for goal_dir, display in goals:
        ...
        # R^WM pooled std via law of total variance:
        #   Var(X) = E_seed[Var(X|seed)] + Var_seed(E[X|seed]).
        # The between-seed term is undefined for n_seeds=1, so drop it there —
        # the within-seed term alone matches R*'s "std across 512 starts".
        R_wm = float(r_wm_means.mean())
        within_var = (r_wm_stds ** 2).mean()
        between_var = float(r_wm_means.var(ddof=1)) if len(r_wm_means) > 1 else 0.0
        R_wm_std = float(np.sqrt(within_var + between_var))
...
def _plot_env(ax, df, title, title_x=0.5,
    ...
    ax.set_title(title, pad=9, fontweight="normal", x=title_x)
    ax.set_ylim(0, 0.8)
...
def main():
    ...
    p.add_argument(
        ...
        help="seeds_<TS> dir for Reacher. If omitted, a fresh sweep is "
             "trained (see --num_seeds).")
    p.add_argument(
        "--mc_sweep", default=None,
        help="seeds_<TS> dir for MountainCar-position. If omitted, a fresh "
             "sweep is trained (see --num_seeds).")
    p.add_argument(
        "--num_seeds", type=int, default=10,
        help="Seeds to train (when auto-training) and read from each sweep. "
             "Default 10 matches the paper headlines; pass 1 for a cheap "
             "reproduction.")
    p.add_argument(
        "--out_dir", default="outputs/results",
        help="Where to write the rendered figure.")
    p.add_argument("--out_stem", default="fig3_unseen")
    args = p.parse_args()

    reacher_sweep = args.reacher_sweep or _train_sweep("reacher", args.num_seeds)
    mc_sweep      = args.mc_sweep      or _train_sweep("mountaincar-position", args.num_seeds)
    print(f"Reacher sweep : {reacher_sweep}")
    print(f"MC      sweep : {mc_sweep}")

    rows = (
        _aggregate_env("Reacher",     reacher_sweep, REACHER_GOALS, args.num_seeds)
        + _aggregate_env("MountainCar", mc_sweep,      MC_GOALS,      args.num_seeds)
    )
