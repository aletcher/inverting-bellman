...
Usage:
    python run.py --config configs/mountaincar-position.py
    python run.py --config configs/mountaincar-position.py --phases pqn,plot_pqn
    python run.py --config configs/mountaincar-position.py --LR 5e-4 --GAMMA 0.95
"""
...
NUM_GOALS = 4
GOALS = jnp.column_stack([jnp.linspace(-1.2, 0.6, NUM_GOALS), jnp.zeros(NUM_GOALS)])
GOAL_MASKS = jnp.tile(jnp.array([1.0, 0.0]), (NUM_GOALS, 1))
GOAL_INPUT_DIMS = [0]
...
UNSEEN_GOALS = [
    # (i) Fast car: reach velocity v = 0.07.
    {
        "label": "Fast car",
