...
Default --num_seeds 10 reproduces the paper-headline numbers (Q_MSE,
WM_MSE, per-unseen-goal R*/R_WM, mean ± SE across seeds) and writes
aggregate.json + summary.txt next to the per-seed dirs. Pass
--num_seeds 1 for a single-seed development run (no aggregation).
...
Usage:
    ...
    uv run python run.py --config configs/reacher.py --num_seeds 1 --phases pqn,plot_pqn
    uv run python run.py --config configs/reacher.py --num_seeds 1 --LR 5e-4 --GAMMA 0.95
    uv run python run.py --config configs/reacher.py --num_seeds 1 --phases wm,plot_wm --pqn_checkpoint outputs/...
"""
...
_WM_AVG_MAPE_RE = re.compile(r"^Avg MAPE:\s*([0-9.eE+\-]+)", re.M)
_VI_AVG_RE = re.compile(
    r"^Avg V_mse=([0-9.eE+\-]+)\s+V_mape=([0-9.eE+\-]+)\s*\n"
    r"Avg Q_mse=([0-9.eE+\-]+)\s+Q_mape=([0-9.eE+\-]+)",
    re.M,
)
_VI_SUPPORT_RE = re.compile(r"^Eval support:\s*(\S.*)$", re.M)
...
def _parse_wm_results(results_path):
    ...
    out = {}
    for key, rx in (("wm_mse", _WM_AVG_MSE_RE), ("wm_mape", _WM_AVG_MAPE_RE)):
        m = rx.search(txt)
        if m:
...
def _parse_vi_results(vi_summary_path):
    ...
    out = {
        ...
        "v_mape": float(m.group(2)),
        "q_mse":  float(m.group(3)),
        "q_mape": float(m.group(4)),
    }
    m_support = _VI_SUPPORT_RE.search(txt)
    if m_support:
        out["eval_support"] = m_support.group(1).strip()
    return out


def _parse_unseen_results(results_path):
    """Pull only the discounted-return row (R*, R_WM) — the paper headline."""
    if not results_path.exists():
        return None
    txt = results_path.read_text()
    true_vals, wm_vals = [], []
    ...
    for line in txt.splitlines():
        ...
        if not in_disc:
            continue
        if stripped.startswith("True policy") and ":" in stripped:
            try:
                true_vals.append(float(stripped.split(":", 1)[1].split("±")[0].strip().split()[0]))
            except Exception:
                pass
        elif stripped.startswith("WM policy") and ":" in stripped:
            try:
                wm_vals.append(float(stripped.split(":", 1)[1].split("±")[0].strip().split()[0]))
            except Exception:
                pass
        elif stripped == "":
            in_disc = False
    if not true_vals or not wm_vals:
        return None
    return {"r_star": true_vals[0], "r_wm": wm_vals[0]}
...
def _aggregate(seed_dirs):
    ...
    for sd in seed_dirs:
        for vi_dir in sorted(sd.glob("vi*")):
            ...
            if parsed:
                ...
                vi_buf["q_mape"].append(parsed["q_mape"])
                if "eval_support" in parsed:
                    vi_supports.append(parsed["eval_support"])
                break
        for wm_dir in sorted(sd.glob("wm_*")):
            ...
            if parsed:
                ...
                wm_buf["wm_mape"].append(parsed.get("wm_mape", float("nan")))
                break
                ...
                for goal_dir in sorted(unseen_root.iterdir()):
                    ...
                    if parsed:
                        bucket = unseen_buf.setdefault(
                            goal_dir.name, {"r_star": [], "r_wm": []}
                        )
                        bucket["r_star"].append(parsed["r_star"])
                        bucket["r_wm"].append(parsed["r_wm"])
    ...
    if vi_supports:
        out["vi"]["eval_support"] = vi_supports[0]
    ...
    for label, bucket in unseen_buf.items():
        entry = {}
        for key in ("r_star", "r_wm"):
            vals = bucket[key]
            if vals:
                mean, se = _mean_se(vals)
                entry[key] = {"mean": mean, "se": se, "n": len(vals), "values": vals}
...
def _write_summary_txt(summary, out_path):
    ...
    if summary["vi"]:
        ...
        q_mape = summary["vi"].get("q_mape")
        support = summary["vi"].get("eval_support", "vi_grid")
        if q_mse:
            lines.append(f"Q_MSE  [{support}] = {q_mse['mean']:.4e} ± {q_mse['se']:.2e}")
        if q_mape:
            lines.append(f"Q_MAPE [{support}] = {q_mape['mean']:.4f} ± {q_mape['se']:.4f}")
    if summary["wm"]:
        wm_mse = summary["wm"].get("wm_mse")
        wm_mape = summary["wm"].get("wm_mape")
        if wm_mse:
            lines.append(f"WM_MSE  = {wm_mse['mean']:.4e} ± {wm_mse['se']:.2e}")
        if wm_mape:
            lines.append(f"WM_MAPE = {wm_mape['mean']:.4f} ± {wm_mape['se']:.4f}")
    if summary["unseen"]:
        lines.append("")
...
def _run_multi_seed(args, extras):
    ...
    for src, fields in (("vi", ("q_mse", "q_mape")), ("wm", ("wm_mse", "wm_mape"))):
        for k in fields:
            v = summary[src].get(k)
            if v:
                fmt = ".4e" if "mse" in k else ".4f"
                print(f"  {k.upper():>8s} = {v['mean']:{fmt}} ± {v['se']:.2e}  (n={v['n']})")
    for label, entry in summary["unseen"].items():
...
def main():
    ...
    parser.add_argument(
        ...
        help="Number of seeds to run (default 10, paper-table). Pass 1 for a "
             "single-seed dev run (no aggregation).",
    )
    parser.add_argument(
    ...
    if "vi" in phases:
        if ENV_CONFIG["VI_GRID_RES"] is None:
            ...
            # Q_EVAL_ON_TRAIN_SUPPORT switches Q_MSE eval from the VI grid to
            # the WM training support for a fair side-by-side with WM_MSE.
            q_eval_on_train_support = ENV_CONFIG.get(
                "Q_EVAL_ON_TRAIN_SUPPORT", False
            )
            vi_train_support_sampler = None
            if ENV_NAME == "Reacher":
                ...
                if q_eval_on_train_support and WM_CONFIG.get("SAMPLE_FROM_RESET", False):
                    # env.reset() returns 6D obs; project to 4D effective.
                    def vi_train_support_sampler(rng, n):
                        rngs = jax.random.split(rng, n)
                        obs_6d = jax.vmap(
                            lambda r: basic_env.reset(r, env_params)[0]
                        )(rngs)
                        return reacher_obs_to_effective(obs_6d)
            else:
            ...
            run_all_goals(
                ...
                train_support_sampler=vi_train_support_sampler,
            )
