...
Numbers below are from a preliminary single-seed MountainCar run; they will be
refreshed as mean ± SE over 5 seeds before submission (the trends are stable).
No figures are embedded here — reference the new figure/appendix by label.
...
ρ(Q_NMSE, WM_NMSE) ≈ 0.8 (p < 1e-5) — and, consistent with our main results, the
recovered model stays one to two orders of magnitude more accurate than the
Q-function it is extracted from (at convergence, WM_NMSE ≈ 7e-3 vs Q_NMSE ≈ 0.18).
New Fig. [X] (MountainCar).
...
optimal-Q error is ≈5× smaller and its recovered-model error ≈30× smaller — i.e.
the on-support regime, not the uniform worst case, is the operative one. (We also
note the recovered model is markedly more accurate than Q even where the
Q-function itself is not especially accurate — a strong form of the "world model
≫ Q-values" phenomenon.)
