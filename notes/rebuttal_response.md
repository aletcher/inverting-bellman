...
*Theory.* Writing $S_o = \bigcup_g \mathrm{Supp}(\pi_g)$ for the set of states visited by an agent (pooled across all goals), the MDP restricted to $S_o \times A$ is a reduced MDP, and our approximate-recovery bound applies verbatim with the on-support error $\varepsilon_o = \sum_{s \in S_o} \| Q(s,a,g) - Q^\pi(s,a,g) \|$ and $M$ restricted to $S_o$:

$$\|\hat{P}(s,a) - P(s,a)\|_1 \;\le\; \|M_{S_o}^{+}\|_1\,(1 + \gamma m)\,\varepsilon_o, \qquad s \in S_o,$$

with identifiability now requiring only $|\mathcal{G}| \ge |S_o|$ rather than $|\mathcal{G}| \ge |\mathcal{S}|$.
