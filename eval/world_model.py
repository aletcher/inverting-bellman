...
def compare_dynamics(p_params, out_dir, wm_config, env_config, dynamics_fn,
                     ...
                     wm_output_dim=None):
    """WM-vs-true dynamics comparison: per-(action, dim) MSE/MAPE + quiver plot.
    ...
    if sample_states_fn is not None:
        ...
        eval_support_label = "WM training sampler (env.reset)"
    else:
        ...
        eval_support_label = "uniform on STATE_RANGES"

    dynamics_metrics = {}
    print(f"\n── Dynamics metrics ({eval_support_label}, {n_eval} states) ──")
    for a in range(action_dim):
        ...
        for d in range(state_dim):
            ...
            abs_true = jnp.abs(s_true[:, d])
            mape_d = float(jnp.mean(
                jnp.where(abs_true > 1e-6, abs_err / abs_true, 0.0)
            ))
            dynamics_metrics[f"{action_names[a]}_{state_labels[d]}"] = {
                "mse": mse_d, "mape": mape_d,
            }
            parts.append(f"{state_labels[d]}: MSE={mse_d:.2e} MAPE={mape_d:.3f}")
        print(f"  {action_names[a]:>5s}: {', '.join(parts)}")
    avg_mse = sum(m["mse"] for m in dynamics_metrics.values()) / len(dynamics_metrics)
    avg_mape = sum(m["mape"] for m in dynamics_metrics.values()) / len(dynamics_metrics)
    ...
    print(f"\nAvg: MSE={avg_mse:.2e} MAPE={avg_mape:.3f}")

    full_slice_label = _slice_label(slice_point, state_labels)
    with open(f"{out_dir}/results.txt", "w") as f:
        ...
        f.write(f"Eval-support source: {eval_support_label}\n")
        f.write(f"Slice point (off-axis values for sweeps/plots): {full_slice_label}\n")
        f.write("\n")
        if losses is not None:
            f.write(f"WM training loss: {float(losses[0]):.6e} -> {float(losses[-1]):.6e}\n\n")
        f.write(f"{'action_dim':<20} {'MSE':>12} {'MAPE':>10}\n")
        for key, m in dynamics_metrics.items():
            f.write(f"{key:<20} {m['mse']:>12.4e} {m['mape']:>10.4f}\n")
        f.write(f"\nAvg MSE: {avg_mse:.6e}\n")
        f.write(f"Avg MAPE: {avg_mape:.4f}\n")
