...
def _precompute_rewards_dones_multi(
    ...
    rew_scale, state_dim, env_terminated_fn,
):
    ...
    def _per_action(ns):
        ...
        return rew_scale * r.reshape(grid_shape), d.astype(float).reshape(grid_shape)
...
def evaluate_single_unseen_goal(
    ...
    if is_multi:
        ...
        all_rewards_wm, all_dones_wm = _precompute_rewards_dones_multi(
            ...
            rew_scale, state_dim, goal_env_terminated_fn,
        )
    ...
    if not skip_per_goal_artifacts:
        if state_dim == 2:
            ...
            # Save V grids for downstream plots / orchestrators.
            np.save(f"{goal_dir}/V_true.npy", np.asarray(V_true))
            np.save(f"{goal_dir}/V_wm.npy", np.asarray(V_wm))
